...
CAP1203::CAP1203(i2c_master_bus_handle_t bus_handle)
    : _bus_handle(bus_handle), _dev_handle(nullptr), _last_button_status(0),
      _callback(nullptr), _callback_user_data(nullptr) {
...
esp_err_t CAP1203::init() {
  ...
  ret = writeRegister(CAP1203_REG_SENSOR_INPUT_ENABLE,
                      0x07); // Enable CS1, CS2, CS3
  if (ret != ESP_OK) {
  ...
  // Enable interrupts so the INT bit latches touches and status clears on
  // release. We still poll for status; the ALERT# pin does not need to be
  // wired.
...
esp_err_t CAP1203::probe() {
  ...
  ESP_LOGI(TAG,
           "CAP1203 found - Product ID: 0x%02X, Manufacturer: 0x%02X, "
           "Revision: 0x%02X",
...
esp_err_t CAP1203::setSensitivity(uint8_t sensitivity) {
  ...
  // Set DELTA_SENSE[2:0] in bits 6-4; keep BASE_SHIFT[3:0] at default 0xF.
...
esp_err_t CAP1203::setInterruptEnable(bool enable) {
  uint8_t value =
      enable ? 0x07 : 0x00; // Enable/disable interrupts for CS1, CS2, CS3
    ...
    if (is_pressed && !was_pressed) {
      ...
      if (!_button_long_fired[i]) {
        if (_callback) {
          ButtonState state;
          state.id = static_cast<ButtonID>(i);
          state.event = (press_duration >= LONG_PRESS_MS)
...
esp_err_t CAP1203::getDeltaValues(int8_t *cs1_delta, int8_t *cs2_delta,
                                  int8_t *cs3_delta) {
  esp_err_t ret;
  uint8_t delta[3];
