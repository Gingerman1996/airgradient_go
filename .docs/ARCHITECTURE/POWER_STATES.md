...
- **BQ25629 QON pin**: For hardware power control
...
1. QON button pressed
       │
       ▼
2. BQ25629 receives QON signal
       │
       ▼
3. BQ25629 exits Ship Mode
       │
       ▼
...
─────────────────────────────────────────
    ...
    │    ├── Initial refresh after first data update (exit TEST SCREEN)
    │    └── Partial refresh at recording interval (default 10s)
    │
    └──► Enter main loop
```

### Runtime Power Path Monitoring

During normal operation, firmware monitors VBUS and PMID to keep the PM sensor stable:
- When VBUS changes to a stable present/absent state, verify SPS30 is still producing recent readings.
- If SPS30 is not reading, read PMID via BQ25629 ADC and attempt recovery:
  - If VBUS is absent and PMID is low (< 4.0V), enable PMID 5V boost.
  - If VBUS is present and PMID is low (< 4.0V), ensure charging is enabled.
- If SPS30 data-ready remains false for more than 2 consecutive read cycles, restart SPS30 measurement.
...
| Temperature | 100.0 | ###.# | 1 decimal place |
| Humidity | 99.9 | ##.# | 1 decimal place |
...
4. Firmware initiates shutdown with status messages:
   ...
   └──► Clear E-paper display (blank screen)
       │
       ▼
5. Send Ship Mode command to BQ25629 via I2C
       │
       ▼
6. BQ25629 enters Ship Mode
       │
       ▼
...
## ⚙️ BQ25629 Ship Mode Command

### I2C Command Sequence
```c
// BQ25629 I2C Address: 0x6A
...
| QON hold | ~12s | BQ25629 enters Sleep Mode automatically |
